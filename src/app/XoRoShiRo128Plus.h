...
public:
    ...
    XoRoShiRo128Plus(const state_type &state) : mState(state) {}
    ...
    // See http://xoroshiro.di.unimi.it/ "Generating uniform doubles in the unit
    // interval"
    double rand01() {
        union {
            uint64_t i;
    ...
    uint64_t operator()(uint64_t bound) {
        ...
        return (static_cast<unsigned __int128>(bound) *
                static_cast<unsigned __int128>(operator()())) >>
               64;
#else
    ...
    static uint64_t max() { return static_cast<uint64_t>(-1); }

    static uint64_t min() { return 0; }

    const state_type &state() const { return mState; }

    void state(const state_type &newState) { mState = newState; }
